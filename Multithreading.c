...
void *professorthread(void *junk) 
{
  ...
  pthread_mutex_lock( &forp );
    while (1) 
  {
    
    if(students_since_break>9 && students_in_office==0)
    {
...
void classa_enter() 
{
  ...
  /*  YOUR CODE HERE.*/
  pthread_mutex_lock( &fora );
  forclassa++;
  if(forclassb!=0){ 
  while(continousa>4 || students_since_break>9 || classb_inoffice>0 || students_in_office>2 )
   {
 
   }
  }
  else{
    while(students_since_break>9 || classb_inoffice>0 || students_in_office>2 )
   {
...
void classb_enter() 
{
  ...
  /* synchronization for the simulations variables below                  */
  /*  YOUR CODE HERE.                                                     */ 
  pthread_mutex_lock( &forb );
  forclassb++;
  if(forclassa!=0){
  while(continousb >4 || students_since_break>9 || classa_inoffice>0 || students_in_office>2)
  {

   }
 }
else{
  while( students_since_break>9 || classa_inoffice>0 || students_in_office>2)
  {
